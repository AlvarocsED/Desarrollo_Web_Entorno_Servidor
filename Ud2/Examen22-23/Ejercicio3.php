...
<body>
    ...
    <?php
      ...
      // Crear un objeto Estancia y guardar en un archivo
      $estancia = new Estancia(
        ...
        $_POST['noches'],
        isset($_POST['cuna']),
        isset($_POST['supletoria']),
        isset($_POST['pago']) && $_POST['pago'] === 'Efectivo'
    );
    
    // Guardar la estancia en un archivo
    $estancia->guardarEstanciaEnArchivo("estancias.txt");
...
class Estancia {
    ...
    private int $noches;
    private string $cuna;
    private string $supletoria;
    private double $pagoEfectivo;

    public function __construct($dni, $nombre, $tipoHabitacion, $tipoEstancia, $noches, $cuna, $supletoria, $pagoEfectivo) {
        $this->dni = $dni;
        $this->nombre = $nombre;
    ...
    public function guardarEstanciaEnArchivo($archivo) {
        $data = $this->dni . ';' . $this->nombre . ';' . $this->tipoHabitacion . ';' . $this->tipoEstancia . ';' . $this->noches . ';' . ($this->cuna ? '1' : '0') . ';' . ($this->supletoria ? '1' : '0') . ';' . ($this->pagoEfectivo ? '1' : '0');
        file_put_contents($archivo, $data . PHP_EOL, FILE_APPEND);
    }
}  
    ?>
</body>
